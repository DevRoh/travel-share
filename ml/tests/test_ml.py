...
test("generate_dataset() returns correct size", test_generate)
test("DataFrame has all required columns",    test_columns)
test("All 4 cities present",                  test_cities)
test("200 records per city",                  test_city_counts)
test("All fares are positive",                test_fare_positive)
test("Fares within reasonable range (₹40–₹5000)", test_fare_reasonable)
test("All distances are positive",            test_distance_positive)
test("Departure hours 0–23",                  test_hour_range)
test("No null values",                        test_no_nulls)
test("Surge table has 24 entries",            test_surge_table)
...
test("train_models() runs without error",     test_training)
test("All 3 models returned",                 test_all_models_returned)
test("All models have MAE < 200",             test_mae_reasonable)
test("Gradient Boosting competitive with LR", test_gb_best)
test("StandardScaler fitted correctly",        test_scaler_fitted)
test("LabelEncoder encodes 4 cities",         test_label_encoder)
...
test("Model artifact files exist",             test_model_files_exist)
test("fare_model.pkl loads correctly",          test_model_loads)
test("meta.json has required keys",             test_meta_json)
test("Prediction within ₹50–₹2000 range",      test_prediction_range)
test("All 4 cities produce valid predictions",  test_all_cities_predict)
test("Fare interval: lower < median < upper",   test_fare_interval)
test("Per-person split always positive",        test_per_person_split)
...
test("Python overlap: identical routes = 1.0",  test_py_identical)
test("Python overlap: score in [0,1]",           test_py_range)
test("Python haversine: symmetric",              test_py_haversine_symmetry)
