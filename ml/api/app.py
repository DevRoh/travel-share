...
def _train_if_needed():
    ...
    if not os.path.exists(pkl):
        print("[WAIT] No trained model found — running generate_dataset.py ...", flush=True)
        ...
        if result.returncode != 0:
            print("[ERROR] Training failed:", result.stderr, flush=True)
        else:
            print("[SUCCESS] Model trained successfully", flush=True)
...
try:
    ...
    print(f"[SUCCESS] ML model loaded — GradientBoosting, MAE approx {meta['metrics']['gradient_boosting']['MAE']}", flush=True)
    MODEL_READY = True
